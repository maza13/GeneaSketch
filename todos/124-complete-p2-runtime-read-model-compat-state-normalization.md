...
commit_message: "Close 124: normalize residual runtime read-model state"
...
- [x] Runtime-facing state no longer implies a meaningful legacy mode branch where none exists.
- [x] Backward-tolerant loading remains intact for old persisted inputs.
- [x] Explicit compatibility edge remains available only where intentionally needed.
...
### 2026-03-07 - Removed runtime read-model mode from store and persistence
...
**Actions:**
- Removed `readModelMode` and `setReadModelMode` from runtime store-facing state so the app no longer advertises a non-existent runtime branch.
- Stopped persisting `readModelMode` in current workspace profile saves while preserving backward-tolerant loading for older profiles.
- Kept hydration normalization returning direct-only semantics and left explicit read-model compatibility tooling untouched in `core/read-model`.
- Updated the live architecture map and workspace profile type comments to reflect that compatibility is now side-edge only.
...
Close umbrella `121` next. The residual cleanup chain is complete and any further work should be opened as a separate follow-up instead of extending this chain.
