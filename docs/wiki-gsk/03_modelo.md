# 03. Modelo de Datos Genraph (Nodos, Aristas y Claims)

## Fuentes de verdad (codigo)
 - Tipos del modelo: `src/core/genraph/types.ts`
 - Validacion: `src/core/genraph/validation.ts`
...
## EdgeType canon
`GEdge.type` debe pertenecer al enum `GenraphEdgeType` definido en `src/core/genraph/types.ts`.
