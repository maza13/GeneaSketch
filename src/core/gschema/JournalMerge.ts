...
export interface JournalMergeResult {
    ...
    mergedOps: GSchemaOperation[];
    conflicts: JournalMergeConflict[];
}

function bySeq(ops: readonly GSchemaOperation[]): Map<number, GSchemaOperation> {
    const m = new Map<number, GSchemaOperation>();
    for (const op of ops) m.set(op.opSeq, op);
    return m;
}

function findCommonAncestorOpSeq(
    base: readonly GSchemaOperation[],
    left: readonly GSchemaOperation[],
    right: readonly GSchemaOperation[]
): number {
    const b = bySeq(base);
    const l = bySeq(left);
    const r = bySeq(right);
...
function opTargetKey(op: GSchemaOperation): string | null {
...
export function mergeJournalsThreeWay(
    ...
    const dedupTail: GSchemaOperation[] = [];
