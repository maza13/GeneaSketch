/**
 * GSchema 0.2.x - GSK Package Serializer/Deserializer
 */
...
import { validateGSchemaGraph } from "./validation";
...
function nextQuarantineOpSeq(graphData: GSchemaGraphData, journalOps: ReturnType<typeof parseJournalFromJsonl>): number {
...
function migrateLegacyClaimModel(graphData: GSchemaGraphData): void {
...
function migrateLegacyQuarantineModel(graphData: GSchemaGraphData): void {
...
export async function importGskPackage(
    ...
    const syntheticRepairOps: GSchemaOperation[] = [];
    ...
    let graphData: GSchemaGraphData | null = null;
    if (!graphRaw) {
        warnings.push("Missing graph.json");
    } else {
        try {
            graphData = JSON.parse(graphRaw) as GSchemaGraphData;
            if (manifest.schemaVersion.startsWith("0.1.")) {
    ...
    if (graphData) {
        ...
        const rawValidation = validateGSchemaGraph(graphData);
    ...
    let graph: GSchemaGraph | null = graphData ? GSchemaGraph.fromData(graphData, snapshotJournalOps) : null;
