...
function isParentChild(edge: GenraphEdge): edge is ParentChildEdge {
    return edge.type === "ParentChild";
}

function isMember(edge: GenraphEdge): edge is MemberEdge {
...
function uniqueEdgeUid(edges: Record<string, GenraphEdge>, base: string): string {
    let candidate = base;
    let i = 1;
...
export function countMissingParentChildUnionLinks(data: GenraphGraphData): number {
    let count = 0;
...
export function ensureParentChildUnionLinks(data: GenraphGraphData): ParentChildUnionRepairResult {
