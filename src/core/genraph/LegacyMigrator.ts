/**
 * Genraph 0.1.x — Legacy Migrator
 *
 * Converts existing .gsz (GeneaSketch Zip, format 0.3.x) files into the new
 * .gsk (Genraph 0.1.x) format. This is a one-way, lossless migration.
 *
 * Migration strategy:
 * 1. Read the .gsz (GEDCOM + embedded media + gskMeta) using existing parser.
 * 2. Run documentToGenraph() to build the GenraphGraph.
 ...
 * genraphToDocument() → serializeGedcom(). The resulting GEDCOM should be
...
import { documentToGenraph, genraphToDocument } from "./GedcomBridge";
...
export async function migrateToGsk(input: MigrationInput): Promise<MigrationResult> {
    ...
    // Step 1: Convert GeneaDocument → GenraphGraph
    const { graph, quarantineCount } = documentToGenraph(
        input.document,
    ...
    if (quarantineCount > 0) {
        warnings.push(`${quarantineCount} tag(s) could not be mapped to Genraph and were quarantined. Check quarantine.json in the .gsk package.`);
    }

    // Step 2: Validate round-trip (projectability check)
    const roundTripDoc = genraphToDocument(graph);
