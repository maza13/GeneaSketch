/**
 * Genraph 0.1.x — GedcomBridge
 *
 * Bidirectional conversion between a `GeneaDocument` (the current internal
 * format, produced by the existing GEDCOM parser) and a `GenraphGraph`.
 *
 ...
 *
 *   IMPORT: File → parser.ts → GeneaDocument → GedcomBridge → GenraphGraph
 *   EXPORT: GenraphGraph → GedcomBridge → GeneaDocument → serializer.ts → File
 *
 * This design is:
...
function collectNonPreferredClaimMarkers(graph: GenraphGraph, nodeUid: string): string[] {
 ...
 */
function decodeConflictMarker(graph: GenraphGraph, nodeUid: string, raw: string): boolean {
...
// ─────────────────────────────────────────────
// GeneaDocument → GenraphGraph  (IMPORT PATH)
// ─────────────────────────────────────────────

export interface BridgeImportResult {
    graph: GenraphGraph;
    /** XREF to Genraph node UID mapping for verification and index. */
    xrefMap: Record<string, string>;
...
/**
 * Converts a GeneaDocument (from the GEDCOM parser) into a GenraphGraph.
 *
 ...
 */
export function documentToGenraph(
    doc: GeneaDocument,
...
// ─────────────────────────────────────────────
// GenraphGraph → GeneaDocument  (EXPORT PATH)
// ─────────────────────────────────────────────

/**
 * Converts a GenraphGraph back into a GeneaDocument.
 *
 ...
 */
export function genraphToDocument(
    graph: GenraphGraph,
    ...
    if (repair.repairedEdges > 0) {
        ...
        graph = GenraphGraph.fromData(exportData, graph.getJournal());
    }
...
/**
 * Round-trip: GeneaDocument → GenraphGraph → GeneaDocument.
...
export function roundTripDocument(doc: GeneaDocument): GeneaDocument {
    ...
    return genraphToDocument(graph);
}
