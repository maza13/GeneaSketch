/**
 * Genraph 0.3.x — GenraphGraph Class
 *
...
import {
    type GenraphGraph as GenraphGraphData,
    type GenraphNode,
    type GenraphEdge,
    type GClaim,
    type GenraphOperation,
...
import { validateGenraphGraph, type ValidationResult } from "./validation";
...
function indexEdge(index: AdjacencyIndex, edge: GenraphEdge): void {
...
function deIndexEdge(index: AdjacencyIndex, edge: GenraphEdge): void {
...
export class GenraphGraph {
    private _nodes: Map<string, GenraphNode> = new Map();
    private _edges: Map<string, GenraphEdge> = new Map();
    ...
    private _journal: GenraphOperation[] = [];
    ...
    static create(): GenraphGraph {
        return new GenraphGraph();
    }

    /**
     * Reconstruct a GenraphGraph from its serialized data (e.g., from a .gsk file).
     * Replays operations to rebuild indexes.
     */
    static fromData(data: GenraphGraphData, journalOps: readonly GenraphOperation[] = []): GenraphGraph {
        const g = new GenraphGraph();
    ...
    private _recordOperation<T extends GenraphOperation>(opWithoutSeq: Omit<T, "opSeq">): T {
    ...
    getParents(personUid: string): Array<{ parent: GenraphNode; edge: ParentChildEdge }> {
        return this.edgesTo(personUid)
            ...
            .filter((x): x is { parent: GenraphNode; edge: ParentChildEdge } => x !== null);
    }

    /** Returns all children of a person (via ParentChild edges). */
    getChildren(personUid: string): Array<{ child: GenraphNode; edge: ParentChildEdge }> {
        return this.edgesFrom(personUid)
            ...
            .filter((x): x is { child: GenraphNode; edge: ParentChildEdge } => x !== null);
    }

    /** Returns all unions (marriages/partnerships) a person belongs to. */
    getUnions(personUid: string): Array<{ union: GenraphNode; edge: MemberEdge }> {
        return this.edgesFrom(personUid)
            ...
            .filter((x): x is { union: GenraphNode; edge: MemberEdge } => x !== null);
    }

    /** Returns all members of a union. */
    getMembers(unionUid: string): Array<{ person: GenraphNode; edge: MemberEdge }> {
        return this.edgesTo(unionUid)
            ...
            .filter((x): x is { person: GenraphNode; edge: MemberEdge } => x !== null);
    }

    // ── Mutations ─────────────────────────────

    addNode(nodeInput: Omit<GenraphNode, "createdAt">, actorId = "system"): GenraphNode {
        const node: GenraphNode = { ...nodeInput, createdAt: nowIso() } as GenraphNode;
    ...
    addEdge(edgeInput: Omit<GenraphEdge, "createdAt">, actorId = "system"): GenraphEdge {
        const edge: GenraphEdge = { ...edgeInput, createdAt: nowIso() } as GenraphEdge;
     ...
     */
    _applyOperation(op: GenraphOperation): void {
        switch (op.type) {
            case "ADD_NODE":
    ...
    _appendJournal(ops: readonly GenraphOperation[], updateJournalArray: boolean): void {
    ...
    _replaceJournal(ops: readonly GenraphOperation[]): void {
        this._journal = [...ops];
        const maxOpSeq = ops.reduce((max, op) => Math.max(max, op.opSeq), -1);
        this._nextOpSeq = maxOpSeq + 1;
    }

    getJournal(): readonly GenraphOperation[] {
    ...
    validate(): ValidationResult {
        return validateGenraphGraph(this.toData());
    }
     ...
     * @internal
     */
    _equals(other: GenraphGraph): boolean {
