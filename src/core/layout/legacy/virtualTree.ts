...
} from "@/core/layout/legacy/orderRules";
...
function buildFamilyDescendantBranches(
  ...
  for (const junctionId of junctions) {
    const junctionNode = makeEntityNode(ctx, junctionId, "generic");
    ...
    for (const childId of children) {
      ...
      const childRow = buildDescendantRow(ctx, childId, false, depth + 1);
...
function buildDescendantRow(
  ...
  const row = makeRowNode(ctx, "down-row", personId, "union-chain");
  const personNode = makeEntityNode(ctx, personId, "union-chain");
  ...
  for (const familyId of families) {
    ...
    const spouses = sortSpousesForFamily(ctx.model, familyId, personId);
    for (const spouseId of spouses) {
      row.children.push(makeEntityNode(ctx, spouseId, "union-chain"));
    }

    buildFamilyDescendantBranches(ctx, familyNode, familyId, depth);
  }
...
function buildSecondaryFamilyBlock(
  ...
  buildFamilyDescendantBranches(ctx, familyNode, familyId, depth + 1);
...
function buildAncestorTree(
  ...
  for (const siblingId of siblings) {
    const siblingNode = makeEntityNode(ctx, siblingId, "siblings");
    siblingNode.sidePreferenceContext = "sibling";
    junctionNode.children.push(siblingNode);
  }
  ...
  const centerFamily = junctionNode.children.find((node) => node.sidePreferenceContext === "centerFam");
  const siblingsBand = junctionNode.children.filter((node) => node.sidePreferenceContext !== "centerFam");

  if (centerFamily) {
    if (lineSide === "paternal") {
      junctionNode.children = [...siblingsBand, centerFamily];
    } else if (lineSide === "maternal") {
      junctionNode.children = [centerFamily, ...siblingsBand];
    } else {
      const split = Math.floor(siblingsBand.length / 2);
      junctionNode.children = [
        ...siblingsBand.slice(0, split),
        centerFamily,
        ...siblingsBand.slice(split)
      ];
    }
  }
