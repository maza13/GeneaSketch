...
function extractOperationTypesFromTypesFile(source: string): Set<string> {
    const blockMatch = source.match(/export type GSchemaOperationType =([\s\S]*?);/);
...
describe("Wiki GSchema operations parity", () => {
    it("keeps 04_operaciones operation list aligned with types.ts", () => {
        const typesSource = fs.readFileSync(path.resolve("src/core/gschema/types.ts"), "utf8");
