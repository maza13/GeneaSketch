/**
 * Genraph Golden Tests
 *
 * These are the canonical correctness tests for the Genraph 0.1.x engine.
 * They verify:
 * 1. Core graph CRUD (add nodes, edges, claims)
 * 2. Graph integrity validation
 * 3. GedcomBridge round-trip (GeneaDocument ↔ GenraphGraph)
...
import { validateGenraphGraph } from "../core/genraph/validation";
import { PersonPredicates, UnionPredicates } from "../core/genraph/predicates";
import { documentToGenraph, genraphToDocument, roundTripDocument } from "../core/genraph/GedcomBridge";
import { serializeJournalToJsonl, parseJournalFromJsonl, replayJournal, applyJournalOps } from "../core/genraph/Journal";
import { exportGskPackage, importGskPackage } from "../core/genraph/GskPackage";
import { mergeJournalsThreeWay } from "../core/genraph/JournalMerge";
...
// ─────────────────────────────────────────────
// 1. GenraphGraph Core Engine
// ─────────────────────────────────────────────

describe("GenraphGraph — core CRUD", () => {
  let graph: GenraphGraph;

  beforeEach(() => {
    graph = GenraphGraph.create();
  });
...
describe("validateGenraphGraph", () => {
  it("should pass validation for an empty graph", () => {
    const graph = GenraphGraph.create();
    const result = validateGenraphGraph(graph.toData());
  ...
  it("should warn when multiple preferred claims conflict", () => {
    ...
    const result = validateGenraphGraph(data);
...
describe("GedcomBridge — round trip", () => {
  it("should convert GeneaDocument to GenraphGraph without data loss", () => {
    const doc = makeMinimalDoc();
    const { graph, xrefMap, quarantineCount } = documentToGenraph(doc, "5.5.1");
  ...
  it("should preserve birth date through GedcomBridge", () => {
    const doc = makeMinimalDoc();
    const { graph, xrefMap } = documentToGenraph(doc, "5.5.1");
  ...
  it("should project GenraphGraph back to GeneaDocument with same persons", () => {
    const doc = makeMinimalDoc();
    const { graph } = documentToGenraph(doc, "5.5.1");
    const projected = genraphToDocument(graph);
  ...
  it("should write unionUid in ParentChild edges during document->genraph projection", () => {
    const doc = makeMultiUnionDoc();
    const { graph, xrefMap } = documentToGenraph(doc, "7.0.x");
  ...
  it("should infer GIVN and SURN claims from bare NAME string with slashes", () => {
    ...
    const { graph } = documentToGenraph(document!);
...
describe("GenraphGraph — toData / fromData", () => {
...
describe("Phase D3: Dirty Data & Quarantine Scenarios", () => {
  it("quarantines unknown tags without failing import", () => {
    ...
    const { graph, quarantineCount } = documentToGenraph(doc, "5.5.1");

    // The graph should still be valid
    const validation = validateGenraphGraph(graph.toData());
...
describe("Journal - Fase 1 determinismo", () => {
  ...
  it("applyJournalOps applies only missing tail ops for fast-forward", () => {
    const source = GenraphGraph.create();
    const personUid = uid();
...
describe("Fase 3 - Quarantine AST + Export conflictos", () => {
  it("imports GEDCOM 5.5.1 unknown tags into quarantine AST preserving hierarchy", () => {
    ...
    const projected = documentToGenraph(parsed.document!, "5.5.1");
  ...
  it("imports GEDCOM 7.x extensions into quarantine AST preserving context", () => {
    ...
    const parsed = parseGedcomAnyVersion(raw);
    expect(parsed.document).toBeTruthy();
    const projected = documentToGenraph(parsed.document!, "7.0.x");
    const quarantine = projected.graph.getQuarantine();
  ...
  it("exports non-preferred claims as NOTE _GSK_CONFLICT for GEDCOM 5.5.1", () => {
    ...
    const doc = genraphToDocument(graph, "5.5.1");
  ...
  it("exports non-preferred claims as _GSK_ALT for GEDCOM 7.0.x", () => {
    ...
    const doc = genraphToDocument(graph, "7.0.x");
...
describe("Fase 4 - Citations first-class + evidence gate", () => {
  it("imports GEDCOM SOUR into claim citations", () => {
    ...
    const parsed = parseGedcomAnyVersion(raw);
    expect(parsed.document).toBeTruthy();
    const { graph, xrefMap } = documentToGenraph(parsed.document!, "7.0.x");
  ...
  it("preserves citations across GEDCOM round-trip", () => {
    ...
    const g1 = documentToGenraph(parsed1.document!, "7.0.x");
    const d2 = genraphToDocument(g1.graph, "7.0.x");
    const ged2 = serializeGedcom(d2, { version: "7.0.3" });
    const parsed2 = parseGedcomAnyVersion(ged2);
    const g2 = documentToGenraph(parsed2.document!, "7.0.x");
...
describe("Fase 5 - container security + journal sync", () => {
  ...
  it("merges two journals with common ancestor into a valid sequence", () => {
    const nodeUid = uid();
    const base = GenraphGraph.create();
...
describe("Extra interoperabilidad y Coerción (Issue 021)", () => {
  it("coerces informal dates and flags isInformal", () => {
    ...
    const { graph, warnings } = documentToGenraph(doc, "5.5.1");
  ...
  it("exports informal union date/place from typed raw values with optional mirror note", () => {
    ...
    const projected = genraphToDocument(graph, "7.0.x");
