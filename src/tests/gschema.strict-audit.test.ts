...
describe("GSchema strict-lossless-audit mode", () => {
    it("rejects invalid journal in strict-lossless-audit while strict-lossless can continue", async () => {
        ...
        const strictLossless = await importGskPackage(payload, { mode: "strict-lossless" });
        expect(strictLossless.warnings.some((w) => w.includes("journal.jsonl contains malformed lines."))).toBe(true);

        await expect(importGskPackage(payload, { mode: "strict-lossless-audit" })).rejects.toThrow("JOURNAL_HASH_MISMATCH");
    });
