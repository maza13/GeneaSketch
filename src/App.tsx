...
export function App() {
  ...
  useEffect(() => {
    shortcutActionsRef.current = {
      ...
      focusSearch: () => {
        const search = window.document.getElementById("person-search") as HTMLInputElement | null;
        search?.focus();
      },
      save: () => saveGsz(),
  ...
  }, [saveGsz, goBack, goForward, fitToScreen]);
  ...
  const nodeMenuState = useMemo(() => {
    ...
    if (nodeMenu.nodeKind === "person") {
      ...
      const items: NodeActionMenuItem[] = [
        ...
        { id: "select-person", label: "🔍 Seleccionar persona", group: "accion", onSelect: () => setSelectedPerson(person.id) },
    ...
    if (family.husbandId && document.persons[family.husbandId]) {
      parentItems.push({
        ...
        onSelect: () => setSelectedPerson(family.husbandId!)
      });
    }
    if (family.wifeId && document.persons[family.wifeId]) {
      parentItems.push({
        ...
        onSelect: () => setSelectedPerson(family.wifeId!)
      });
    }
    ...
    const childItems = family.childrenIds.reduce<NodeActionMenuItem[]>((acc, childId) => {
      ...
      if (child) acc.push({ id: `select-child-${child.id}`, label: `👶 ${child.name}`, onSelect: () => setSelectedPerson(child.id) });
      return acc;
    }, []);
  ...
  }, [nodeMenu, document, viewConfig, setSelectedPerson, clearOverlayType, setOverlay, setStatus, toggleDTreeNodeCollapse, setFocusFamilyId]);
  ...
  const menus: MenuGroup[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: "panel-details", label: "Panel: detalles", checked: viewConfig?.rightPanelView === "details", disabled: !viewConfig, onClick: () => setRightPanelView("details") },
        { id: "panel-timeline", label: "Panel: timeline", checked: viewConfig?.rightPanelView === "timeline", disabled: !viewConfig, onClick: () => setRightPanelView("timeline") },
        { id: "scope-visible", label: "Alcance: visibles", checked: viewConfig?.timeline.scope === "visible", disabled: !viewConfig, onClick: () => setTimelineScope("visible") },
        { id: "scope-all", label: "Alcance: todo", checked: viewConfig?.timeline.scope === "all", disabled: !viewConfig, onClick: () => setTimelineScope("all") },
        { id: "timeline-list", label: "Timeline: lista", checked: viewConfig?.timeline.view === "list", disabled: !viewConfig, onClick: () => setTimelineView("list") },
        { id: "timeline-scale", label: "Timeline: escala", checked: viewConfig?.timeline.view === "scale", disabled: !viewConfig, onClick: () => setTimelineView("scale") },
        { id: "layout-vnext", label: "Layout: vnext", checked: (viewConfig?.dtree?.layoutEngine ?? "vnext") === "vnext", disabled: !viewConfig, onClick: () => setDTreeLayoutEngine("vnext") },
        { id: "layout-v2", label: "Layout: v2", checked: (viewConfig?.dtree?.layoutEngine ?? "vnext") === "v2", disabled: !viewConfig, onClick: () => setDTreeLayoutEngine("v2") },
        { id: "layout-legacy", label: "Layout: legacy", checked: (viewConfig?.dtree?.layoutEngine ?? "vnext") === "legacy", disabled: !viewConfig, onClick: () => setDTreeLayoutEngine("legacy") }
      ]
    },
    {
      id: "tools",
  ...
  return (
    <div className="app" style={{ "--canvas-bg-custom": colorTheme.background, "--node-fill-custom": colorTheme.personNode, "--node-text-custom": colorTheme.text, "--edge-color-custom": colorTheme.edges } as React.CSSProperties}>
      ...
      <main className="layout">
        <LeftPanel document={document} selectedPersonId={selectedPersonId} onSelectPerson={setSelectedPerson} onCreatePerson={() => setPersonEditor({ type: "create_standalone" })} />
        <section className="canvas-panel">
          ...
          <DTreeView graph={expandedGraph} document={document} fitNonce={fitNonce} onNodeClick={handleNodeClick} onNodeContextMenu={handleNodeContextMenu} focusPersonId={viewConfig?.focusPersonId ?? null} focusFamilyId={viewConfig?.focusFamilyId ?? null} selectedPersonId={selectedPersonId} colorTheme={colorTheme} dtreeConfig={normalizedDtreeConfig} onBgClick={() => setNodeMenu(null)} onBgDoubleClick={() => { clearAllOverlays(); setSelectedPerson(null); setDetailPersonId(null); }} onSvgReady={(svg) => { graphSvgRef.current = svg; }} />
        </section>

        {viewConfig?.rightPanelView === "timeline" ? (
          <TimelineRightPanel document={document} expandedGraph={expandedGraph} viewConfig={viewConfig} onTimelineView={setTimelineView} onTimelineScaleZoom={setTimelineScaleZoom} onTimelineScaleOffset={setTimelineScaleOffset} onTimelineHighlight={handleTimelineHighlight} onTimelineStatus={setTimelineStatus} />
        ) : (
          <RightPanel document={document} viewConfig={viewConfig} visualConfig={visualConfig} selectedPersonId={detailPersonId || selectedPersonId} onDTreeOrientation={setDTreeOrientation} onPreset={setPreset} onDepth={setDepth} onInclude={setInclude} onEditPerson={openPersonEditor} onViewPersonDetail={(personId) => setDetailPersonId(personId)} onAddRelation={openAddRelationEditor} onLinkExistingRelation={(anchorId, type) => setPicker({ anchorId, type })} onUnlinkRelation={(personId, relatedId, type) => { unlinkRelation(personId, relatedId, type); setStatus(`Relación desvinculada: ${type}`); }} onGridEnabled={setGridEnabled} onClearPositions={clearNodePositions} />
        )}
      </main>
